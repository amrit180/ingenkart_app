...
import { setTestUsername, setisInstaVerified } from "../../redux/userSlice";
          ...
          <Pressable style={{ backgroundColor: colors.white, height: "100%" }}>
            ...
            <View style={[global.center, { marginTop: h(0.02) }]}>
              <View style={global.center}>
                <Icon name={location} size={w(0.06)} />
                <AppText
                  ml={w(0.01)}
            ...
            <View style={{ paddingHorizontal: w(0.05), marginTop: h(0.03) }}>
              ...
              <View style={[global.start, { marginTop: h(0.02) }]}>
                <Icon name={category} size={w(0.06)} />
                <AppText
                  ml={w(0.01)}
                  fontFamily={"Montserrat_500Medium"}
                  fontSize={10}
...
const InstaProfile = ({ user, state, instaData, mine }) => {
  ...
  const handleSubmit = async (username) => {
    try {
      await testinstaUsername(username, user?._id, user?.token).then(() => {
        ...
        dispatch(setisInstaVerified({ isInstaVerified: true }));
        navigation.goBack();
      });
    } catch (e) {
      console.log(e.respose.data);
