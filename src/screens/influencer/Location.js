    ...
    <Layout>
      ...
      >
        ...
        >
          ...
          <AppText
            text={`${"State: " + user?.state + ", " + "City: " + user?.city}`}
            fontFamily={"Montserrat_500Medium"}
            fontSize={15}
          />
        ...
        <Input
          ...
          variant="text"
          // mb={user?.error && location.state?.length < 1 ? 0 : h(0.03)}
          width={0.893}
          fontFamily={"Montserrat_500Medium"}
          fontSize={16}
          maxLength={100}
          textAlign="left"
          // error={user?.error && location.state?.length < 0}
          message="location not found"
        />
        {/* the city input box will be active only when state is selected, untill then it will be disabled */}
        {user?.state !== "" && isStateSelected && (
          <Input
            pH={w(0.025)}
            ...
            variant="text"
            // mb={user?.error && location?.city?.length < 1 ? 0 : h(0.03)}
            width={0.893}
            fontFamily={"Montserrat_500Medium"}
            fontSize={16}
            maxLength={100}
            textAlign="left"
            // error={user?.error && location?.city?.length < 0}
            message="location not found"
          />
        )}
